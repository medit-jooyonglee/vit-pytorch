...
class PatchEmbedding(nn.Module):
    def __init__(self, *, dim, dim_out, patch_size):
        ...
        self.proj = nn.Sequential(
            LayerNorm(patch_size ** 3 * dim),
            nn.Conv3d(patch_size ** 3 * dim, dim_out, 1),
            LayerNorm(dim_out)
        )

    def forward(self, fmap):
        p = self.patch_size
        fmap = rearrange(fmap, 'b c (d p1) (h p2) (w p3) -> b (c p1 p2 p3) d h w', p1 = p, p2 = p, p3 = p)
        return self.proj(fmap)

class PEG(nn.Module):
    def __init__(self, dim, kernel_size = 3):
        super().__init__()
        self.proj = Residual(nn.Conv3d(dim, dim, kernel_size = kernel_size, padding = kernel_size // 2, groups = dim, stride = 1))
...
class LocalAttention(nn.Module):
    ...
    def forward(self, fmap):
        ...
        b, n, z, x, y, h = *shape, self.heads
        z, x, y = map(lambda t: t // p, (z, x, y))

        fmap = rearrange(fmap, 'b c (z p1) (x p2) (y p3) -> (b z x y) c p1 p2 p3', p1 = p, p2 = p, p3 = p)

        q, k, v = (self.to_q(fmap), *self.to_kv(fmap).chunk(2, dim = 1))
        q, k, v = map(lambda t: rearrange(t, 'b (h d) p1 p2 p3 -> (b h) (p1 p2 p3) d', h = h), (q, k, v))
        ...
        out = rearrange(out, '(b z x y h) (p1 p2 p3) d -> b (h d) (z p1) (x p2) (y p3)', h = h, z = z, x = x, y = y, p1 = p, p2 = p, p3 = p)
...
class GlobalAttention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0., k = 7):
        ...
        self.to_q = nn.Conv3d(dim, inner_dim, 1, bias = False)
        self.to_kv = nn.Conv3d(dim, inner_dim * 2, k, stride = k, bias = False)
    ...
    def forward(self, x):
        ...
        shape = x.shape
        b, n, _, y, z, h = *shape, self.heads
        q, k, v = (self.to_q(x), *self.to_kv(x).chunk(2, dim = 1))

        q, k, v = map(lambda t: rearrange(t, 'b (h d) x y z -> (b h) (x y z) d', h = h), (q, k, v))
