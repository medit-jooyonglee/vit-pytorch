...
class Attention(nn.Module):
    def __init__(self, dim, fmap_size, heads = 8, dim_key = 32, dim_value = 64, dropout = 0., dim_out = None, downsample = False):
        ...
        self.to_q = nn.Sequential(nn.Conv3d(dim, inner_dim_key, 1, stride = (2 if downsample else 1), bias = False), nn.BatchNorm3d(inner_dim_key))
        self.to_k = nn.Sequential(nn.Conv3d(dim, inner_dim_key, 1, bias = False), nn.BatchNorm3d(inner_dim_key))
        self.to_v = nn.Sequential(nn.Conv3d(dim, inner_dim_value, 1, bias = False), nn.BatchNorm3d(inner_dim_value))

        self.attend = nn.Softmax(dim = -1)
        self.dropout = nn.Dropout(dropout)

        out_batch_norm = nn.BatchNorm3d(dim_out)
        nn.init.zeros_(out_batch_norm.weight)

        self.to_out = nn.Sequential(
            nn.GELU(),
            nn.Conv3d(inner_dim_value, dim_out, 1),
            out_batch_norm,
            nn.Dropout(dropout)
        )
        ...
        self.pos_bias = nn.Embedding(fmap_size * fmap_size * fmap_size, heads)

        q_range = torch.arange(0, fmap_size, step = (2 if downsample else 1))
        k_range = torch.arange(fmap_size)

        q_pos = torch.stack(torch.meshgrid(q_range, q_range, q_range, indexing = 'ij'), dim = -1)
        k_pos = torch.stack(torch.meshgrid(k_range, k_range, k_range, indexing = 'ij'), dim = -1)

        q_pos, k_pos = map(lambda t: rearrange(t, 'i j k c -> (i j k) c'), (q_pos, k_pos))
        rel_pos = (q_pos[:, None, ...] - k_pos[None, :, ...]).abs()

        x_rel, y_rel, z_rel = rel_pos.unbind(dim = -1)
        pos_indices = (x_rel * fmap_size * fmap_size) + (y_rel * fmap_size) + z_rel
    ...
    def forward(self, x):
        ...
        out = rearrange(out, 'b h (x y z) d -> b (h d) x y z', h = h, y = y, z = z)
