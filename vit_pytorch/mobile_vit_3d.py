...
class MobileViTBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        _, _, d, h, w = x.shape
        x = rearrange(x, 'b c (d pd) (h ph) (w pw) -> b (pd ph pw) (d h w) c', pd=self.pd, ph=self.ph, pw=self.pw)
        x = self.transformer(x)
        x = rearrange(x, 'b (pd ph pw) (d h w) c -> b c (d pd) (h ph) (w pw)', d=d//self.pd, h=h//self.ph, w=w//self.pw, pd=self.pd, ph=self.ph, pw=self.pw)
